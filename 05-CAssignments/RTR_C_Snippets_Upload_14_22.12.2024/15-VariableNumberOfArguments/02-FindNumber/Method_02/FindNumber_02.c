...
void findNumber(int num_to_be_found_ra, int num_ra, ...) // variadic function
{
    ...
    count_ra = va_findNumber(num_to_be_found_ra, num_ra, num_list_ra);
...
int va_findNumber(int num_to_be_found_ra, int num_ra, va_list list_ra)
{
    int count_of_num_ra = 0;
    int n_ra;

    while (num_ra)
    {
        n_ra = va_arg(list_ra, int);
